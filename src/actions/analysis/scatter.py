...
def _axis_specific_overrides(args, axis: str, fn_name: str) -> Dict:
    ...
    if fn_name == "phrase":
        ...
        if phrase_val:
            ...
            overrides["case_sensitive"] = bool(
                getattr(args, f"{prefix}case_sensitive", False)
            )
...
def _resolve_category_name(
    result_dict: Dict, requested_category: str, fn_name: str, axis: str, args
) -> str:
    """
    ...
    if fn_name == "phrase":
        ...
        if phrase_val:
            actual_key = requested_category.replace(
...
def _compute_points_custom(
    ...
    resolved_x_category = _resolve_category_name(
        x_results, args.x_category, args.x_function, "x", args
    )
    ...
    resolved_y_category = _resolve_category_name(
        y_results, args.y_category, args.y_function, "y", args
    )
...
def _compute_points_lfwt(
    ...
    for member in chat_members:
        ...
        conv_participated = (
            len(pd.unique(msgs["conversation number"])) if len(msgs) > 0 else 0
        )
        participation_rate = helpers.safe_divide_as_pct(
            conv_participated, total_conversations
        )
...
def _scale_expected_to_match_actual_mean(
    ...
    expected_included = [
        v for v, include in zip(expected_values_list, nonzero_mask) if include
    ]
    actual_included = [
        v for v, include in zip(actual_values_list, nonzero_mask) if include
    ]
    ...
    actual_mean = (
        sum(actual_included) / len(actual_included) if actual_included else 0.0
    )
    if expected_mean <= 0:
        return expected_values_list
...
def _compute_points_rroe(
    ...
    for name in chat_members:
        ...
        message_share = _clamp_share(
            (messages_sent / total_original_messages)
        ...
        actual_values_list.append(
            helpers.safe_divide(
                reactions_received_per_member.get(name, 0), messages_sent
            )
        )
    ...
    points = [
        (n, x, y)
        for n, x, y in zip(member_order, expected_values_list, actual_values_list)
    ]
...
def run_scatter(df: pd.DataFrame, args, chat_members: List[str]) -> Dict[str, str]:
    ...
    if preset_lower:
        if preset_lower == "lfwt":
            points, title, subtitle, slug, x_label, y_label = _compute_points_lfwt(
                df, args, chat_members
            )
        elif preset_lower == "rroe":
            points, title, subtitle, slug, x_label, y_label = _compute_points_rroe(
                df, args, chat_members
            )
        else:
        ...
        if not (
            getattr(args, "x_function", None)
            and getattr(args, "x_category", None)
            and getattr(args, "y_function", None)
            and getattr(args, "y_category", None)
        ):
            raise ValueError(
                "Custom scatter requires x-function, x-category, y-function, and y-category"
            )
        points, title, subtitle, slug, x_label, y_label = _compute_points_custom(
            df, args, chat_members
        )
    ...
    return generate_scatter_image(
        ...
        add_residuals=(
            getattr(args, "scatter_regression", False)
            and getattr(args, "scatter_residuals", False)
        ),
        ...
        or getattr(args, "scatter_identity", False),
