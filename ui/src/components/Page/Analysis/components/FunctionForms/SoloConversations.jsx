...
const SoloConversationsForm = ({ scope = "primary" }) => {
  const selectors = (s) => {
    if (scope === "scatter-x") {
      return {
        minutesThreshold: s.scatterXMinutesThreshold,
        setMinutesThreshold: s.setScatterXMinutesThreshold,
        excludeReactions: s.excludeReactions,
        setExcludeReactions: s.setExcludeReactions,
      };
    }
    if (scope === "scatter-y") {
      return {
        minutesThreshold: s.scatterYMinutesThreshold,
        setMinutesThreshold: s.setScatterYMinutesThreshold,
        excludeReactions: s.excludeReactions,
        setExcludeReactions: s.setExcludeReactions,
      };
    }
    return {
      minutesThreshold: s.minutesThreshold,
      setMinutesThreshold: s.setMinutesThreshold,
      excludeReactions: s.excludeReactions,
      setExcludeReactions: s.setExcludeReactions,
    };
  };
      ...
      {scope === "primary" && (
        <label className="flex items-center gap-2 text-sm text-slate-700">
          <input
            ...
            className="h-4 w-4"
          />
          <span>Exclude reactions (i.e. consider a conversation solo even if someone reacts)</span>
        </label>
      )}
    </div>
  );
...
export default SoloConversationsForm;
