...
class MimeType(Function):
    ...
    def process_df(self, df, mime_type):
    ...
    def get_results(self, output_dict, df, mime_type, member_name=None, time_period=None):
    ...
    def get_table_results(self, result_dict, df, chat_members, args):
        ...
        self.process_df(df, mime_type)
        for member_name in chat_members:
            helpers.initialize_member(member_name, result_dict)
            self.get_results(result_dict, df, mime_type, member_name)
    ...
    def get_graph_results(self, graph_data, df, chat_members, time_periods, args):
        ...
        if args.graph_individual:
            self.get_individual_graph_results(
                graph_data, df, mime_type, chat_members, time_periods
            )
        else:
            self.get_total_graph_results(graph_data, df, mime_type, time_periods)

    def get_individual_graph_results(
        self, graph_data, df, mime_type, chat_members, time_periods
    ):
        for time_period in time_periods:
            for member_name in chat_members:
                self.get_results(
                    graph_data[member_name], df, mime_type, member_name, time_period
                )

    def get_total_graph_results(self, graph_data, df, mime_type, time_periods):
        for time_period in time_periods:
            self.get_results(graph_data[GRAPH_TOTAL_KEY], df, mime_type, None, time_period)
