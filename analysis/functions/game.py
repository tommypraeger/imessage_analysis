...
games_column = 'Messages that are games'
percent_games_column = 'Percent of messages that are games'
game_starts_column = 'Messages that start games'
percent_game_starts_column = 'Percent of games started'


def get_columns():
    return [
        games_column,
        percent_games_column,
        game_starts_column,
        percent_game_starts_column
    ]
...
def get_results(output_dict, df, member_name=None, time_period=None):
    ...
    output_dict[games_column].append(game_messages)
    output_dict[percent_games_column].append(
        round(helpers.safe_divide(game_messages, nr_messages) * 100, 2))
    output_dict[game_starts_column].append(game_starts)
    return game_starts
...
def get_table_results(result_dict, df, chat_members, args):
    ...
    total_game_starts = sum(result_dict[game_starts_column])
    for i in range(len(result_dict[game_starts_column])):
        result_dict[percent_game_starts_column].append(round(
            helpers.safe_divide(result_dict[game_starts_column][i], total_game_starts) * 100, 2))
...
def get_individual_graph_results(graph_data, df, chat_members, time_periods):
    for time_period in time_periods:
        ...
        for member_name in chat_members:
            graph_data[member_name][percent_game_starts_column].append(round(helpers.safe_divide(
                graph_data[member_name][game_starts_column][-1], total_games_started_in_period) * 100, 2))
