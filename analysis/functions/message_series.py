...
def get_categories():
    return [message_series_category, average_messages_category, percent_series_category]
...
def get_results(output_dict, df, member_name=None, time_period=None):
    ...
    message_series = len(
        messages[
            (messages["is new message series?"])
...
def get_table_results(result_dict, df, chat_members, args):
    ...
    for i in range(len(result_dict[message_series_category])):
        result_dict[percent_series_category].append(
            round(
                helpers.safe_divide(
                    result_dict[message_series_category][i], total_message_series
                )
                * 100,
                2,
            )
    ...
    for time_period in time_periods:
        ...
        for member_name in chat_members:
            graph_data[member_name][percent_series_category].append(
                round(
                    helpers.safe_divide(
                        graph_data[member_name][message_series_category][-1],
                        total_message_series_in_period,
                    )
                    * 100,
                    2,
                )
