...
message_series_column = 'Total number of message series'
average_messages_column = 'Average messages per series'
percent_series_column = 'Percent of total message series'


def get_columns():
    return [
        message_series_column,
        average_messages_column,
        percent_series_column
    ]
...
def get_results(output_dict, df, member_name=None, time_period=None):
    ...
    output_dict[message_series_column].append(message_series)
    output_dict[average_messages_column].append(
...
def get_table_results(result_dict, df, chat_members, args):
    ...
    total_message_series = sum(result_dict[message_series_column])
    for i in range(len(result_dict[message_series_column])):
        result_dict[percent_series_column].append(
            round(helpers.safe_divide(result_dict[message_series_column]
...
def get_individual_graph_results(graph_data, df, chat_members, time_periods):
    for time_period in time_periods:
        ...
        for member_name in chat_members:
            graph_data[member_name][percent_series_column].append(
                round(helpers.safe_divide(graph_data[member_name][message_series_column][-1], total_message_series_in_period)
                      * 100, 2))
