...
def main(result_dict, df, chat_members):
    ...
    for member_name in chat_members:
        ...
        result_dict['% of messages that are reactions'].append(
            round((1 - helpers.safe_divide(non_reaction_messages, total_messages)) * 100, 2)
        )

        reactions = int(df[df['sender'] == member_name]['reaction action'].sum())
        like_reacts = int(df[df['sender'] == member_name]['like react action'].sum())
        love_reacts = int(df[df['sender'] == member_name]['love react action'].sum())
        dislike_reacts = int(df[df['sender'] == member_name]['dislike react action'].sum())
        laugh_reacts = int(df[df['sender'] == member_name]['laugh react action'].sum())
        emphasis_reacts = int(df[df['sender'] == member_name]['emphasis react action'].sum())
        question_reacts = int(df[df['sender'] == member_name]['question react action'].sum())
    ...
    for i in range(len(result_dict['total messages'])):
        ...
        result_dict['% of all reaction messages that are by this person'].append(
            round(
                helpers.safe_divide(
                    result_dict['reaction messages'][i],
