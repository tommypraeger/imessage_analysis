...
def get_table_results(result_dict, df, chat_members, args):
    ...
    for i in range(len(result_dict[total_messages_category])):
        result_dict[percent_all_reactions_category].append(round(helpers.safe_divide(
            result_dict[reactions_messages_category][i],
            total_reaction_messages) * 100, 2))
        result_dict[percent_all_non_reaction_category].append(round(helpers.safe_divide(
